...
function parseCommits(repoPath) {
    ...
    const raw = (0, child_process_1.execSync)(`git log --pretty=format:"%H${DELIMITER}%ae${DELIMITER}%an${DELIMITER}%at${RAW_SEP}" --name-only`, { cwd: repoPath, stdio: 'pipe', maxBuffer: 512 * 1024 * 1024 }).toString();
    const commits = [];
