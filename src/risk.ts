        ...
        try {
          if (options.hotspots) {
            if (options.since) {
              console.error(
                chalk.red('\n  ✖  Error: ') +
                  '--since cannot be used with --hotspots. Hotspot analysis uses a fixed 12-month recent window for its temporal signal.',
              );
              process.exit(1);
            }
            ...
            const scopeRisks = buildScopeRisks(lifetimeResult);
            const churn = buildContributorChurn(lifetimeResult);
            const abandoned = buildAbandonedScopes(scopeRisks, churn);
            const transitions = buildOwnershipTransitions(lifetimeResult);
            const temporal = buildTemporalScopeRisks(lifetimeResult, recentResult);
            ...
            console.log(
              ` ${chalk.bold.white('⛏  git-arch risk --hotspots')} — ${chalk.grey(resolvedPath.split('/').pop())}`,
            );
            console.log(
              chalk.grey('  Scopes ranked by how many maintenance-risk signals fired'),
            );
            console.log(
              chalk.grey(
                `  Showing scopes with ${minSignals}+ signals${options.all ? '' : ' — use --all for 1+'}`,
              ),
            );
            console.log(chalk.hex('#A78BFA')('─'.repeat(70)) + '\n');

            if (hotspots.length === 0) {
              console.log(chalk.green('  ✓ No scopes met the signal threshold.\n'));
            }

            for (const h of hotspots) {
              console.log(
                chalk.bold.red(`  ${h.signalsFired} signals`) +
                  `  ${chalk.cyan(h.scope)}`,
              );
              for (const s of h.signals) {
                console.log(chalk.grey(`    * ${s.reason}`));
              }
            ...
            console.log(
              chalk.grey(
                '  Each signal reuses analysis already shown by other risk views.',
              ),
            );
            console.log(
              chalk.grey(
                '  These signals do not prove ownership, expertise, or maintainership.',
              ),
            );
            console.log();
            console.log(chalk.hex('#A78BFA')('─'.repeat(70)) + '\n');
          ...
          if (options.temporal) {
            if (options.since) {
              console.error(
                chalk.red('\n  ✖  Error: ') +
                  '--since cannot be used with --temporal. Temporal risk uses a fixed 12-month recent window.',
              );
              process.exit(1);
            }
            ...
            if (options.series) {
              ...
              for (const s of series) {
                ...
                for (const p of s.points) {
                  const value = p.concentration === null ? '-' : `${p.concentration}%`;

                  console.log(`  ${p.year}: ${value} (${p.commitCount} touches)`);
                }

                console.log(chalk.grey(`  Trend: ${s.direction}`));
              }
            ...
            const ownershipTransitions = buildOwnershipTransitions(lifetimeResult);

            const contributorChurn = buildContributorChurn(lifetimeResult);

            const abandonedScopes = buildAbandonedScopes(
            ...
            if (options.html) {
              ...
              generateHtmlReport(lifetimeResult, options.html, temporalRisks, {
                evolutionSummary,
                hotspots: hotspotsForHtml,
              });
            ...
            console.log('\n' + chalk.hex('#A78BFA')('─'.repeat(70)));
            console.log(
              ` ${chalk.bold.white('⛏  git-arch risk --temporal')} — ${chalk.grey(resolvedPath.split('/').pop())}`,
            );
            ...
            for (const r of temporalRisks) {
              const color =
                r.category === 'Persistent concentration' ||
                r.category === 'Emerging concentration'
                  ? chalk.red
              ...
              if (
                r.category === 'No recent activity' ||
                r.category === 'Insufficient recent evidence'
              ) {
                console.log(
                  `  Recent:   ${chalk.bold(String(r.recentTouches))} non-bot file touches`,
                );
              } else if (r.recent) {
                console.log(
                ...
                if (r.delta !== null) {
                  ...
                  console.log(
                    `  Delta:    ${chalk.bold(`${sign}${r.delta} percentage points`)}`,
                  );
                }
              } else {
                console.log(
            ...
            if (contributorChurn.length > 0) {
              console.log(chalk.magenta.bold('  Contributor Churn'));
            ...
            if (abandonedScopes.length > 0) {
              console.log(chalk.magenta.bold('  Potentially Abandoned Areas'));

              console.log();

              for (const a of abandonedScopes) {
                console.log(`  ${chalk.cyan(a.scope)}`);
                ...
                console.log(chalk.grey(`    ${a.explanation}`));

                console.log();
              }
            }
            if (ownershipTransitions.length > 0) {
              console.log(chalk.magenta.bold('  Ownership Transitions'));

              for (const t of ownershipTransitions) {
                console.log(`  ${chalk.cyan(t.scope)}`);
                ...
                console.log(chalk.grey(`    ${t.explanation}`));
            ...
            console.log(
              chalk.grey(
                '  HIGH and MEDIUM are treated as concentrated; LOW is treated as distributed.',
              ),
            );
            console.log(
              chalk.grey(
                '  Recent scopes with 1-9 non-bot touches are marked insufficient recent evidence.',
              ),
            );
            console.log(
              chalk.grey(
          ...
          console.log('\n' + chalk.hex('#A78BFA')('─'.repeat(70)));
          console.log(
            ` ${chalk.bold.white('⛏  git-arch risk')} — ${chalk.grey(resolvedPath.split('/').pop())}`,
          );
          console.log(
            chalk.grey('  Maintenance risk map — not an ownership leaderboard'),
          );
          console.log(
            chalk.grey(
              `  Analysis window: ${since ? `since ${since}` : 'all available history'}`,
            ),
          );
          console.log(chalk.hex('#A78BFA')('─'.repeat(70)) + '\n');
          ...
          for (const r of shown) {
            ...
            console.log(`  ${chalk.cyan(r.scope)}`);
            console.log(
              `  Historical commit-touch concentration: ${chalk.bold(r.concentration + '%')}`,
            );
            ...
            if (r.lastActive) {
              ...
              const inactive =
                r.lastActive.includes('year') || r.lastActive.includes('years');
          ...
          if (!options.all && lowCount > 0) {
            console.log(
              chalk.grey(
                `  ${lowCount} additional scope(s) marked LOW risk — use --all to show them.\n`,
              ),
            );
          }
