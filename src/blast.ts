    ...
    .action(
      async (
        ...
        try {
          ...
          const totalAffected = Array.from(coChanges.values()).filter(
            (v) => Math.round((v / targetCommits.length) * 100) >= 10,
          ).length;
          ...
          console.log();
          console.log(
            `  ${chalk.hex('#A78BFA')('Based on')}       ${chalk.yellow.bold(String(targetCommits.length))} commits touching this file`,
          );
          console.log(
            `  ${chalk.hex('#A78BFA')('Blast radius')}   ${chalk.yellow.bold(String(totalAffected))} files historically change together with it`,
          );
          console.log();

          if (results.length === 0) {
            console.log(
              chalk.green('  ✓ This file changes independently — low blast radius.\n'),
            );
            return;
          }
          ...
          for (const r of results) {
            ...
            console.log(
              `  ${pctColor(bar)} ${pctColor.bold((r.pct + '%').padStart(4))}  ${chalk.white(file)}`,
            );
          }

          console.log();
          console.log(
            chalk.grey('  Percentage = how often this file changed in the same commit'),
          );
          console.log('\n' + chalk.hex('#A78BFA')('─'.repeat(70)) + '\n');
        } catch (err: unknown) {
