...
function buildTree(result: AnalysisResult): object {
  const sm = new Map<string, number>();
  for (const f of result.cursedFiles) sm.set(f.filepath, f.curseScore);
...
<div class="stats">
  ...
  <div class="stat d"><div class="n">${result.busFactor.filter((b: any) => b.busFactor === 1).length}</div><div class="l">Bus Factor 1</div></div>
