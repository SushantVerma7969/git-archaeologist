    ...
    .action((filepath: string, repoPath: string | undefined) => {
      ...
      try {
        ...
        const authorMap = new Map<
          string,
          { name: string; count: number; first: number; last: number }
        >();
        ...
        const prior = fileCommits.filter(
          (c) => c.timestamp >= now - sixMonths * 2 && c.timestamp < now - sixMonths,
        ).length;
        const accel =
          prior === 0
            ? recent > 0
              ...
              : 'stable'
            : recent > prior
              ? `accelerating ${(recent / prior).toFixed(1)}x`
              : recent < prior
                ? 'slowing down'
                : 'stable';
        ...
        console.log(
          ` ${chalk.bold.white('⛏  git-arch blame')} — ${chalk.cyan(filepath)}`,
        );
        console.log(chalk.hex('#A78BFA')('─'.repeat(70)));
        console.log();
        console.log(
          `  ${chalk.hex('#A78BFA')('Total changes')}   ${chalk.yellow.bold(String(totalChanges))}`,
        );
        console.log(
          `  ${chalk.hex('#A78BFA')('Unique authors')}  ${chalk.yellow.bold(String(authors.length))}`,
        );
        console.log(
          `  ${chalk.hex('#A78BFA')('First changed')}   ${chalk.white(firstDate)}`,
        );
        console.log(
          `  ${chalk.hex('#A78BFA')('Last changed')}    ${chalk.white(lastDate)}`,
        );
        console.log(
          `  ${chalk.hex('#A78BFA')('Trend')}           ${recent > prior ? chalk.red(accel) : chalk.green(accel)}`,
        );
        console.log(
          `  ${chalk.hex('#A78BFA')('Recent (6mo)')}   ${chalk.white(String(recent))} commits`,
        );
        console.log(
          `  ${chalk.hex('#A78BFA')('Prior (6mo)')}    ${chalk.white(String(prior))} commits`,
        );
        console.log();
        console.log(chalk.hex('#A78BFA')('─'.repeat(70)));
        ...
        for (const data of authors) {
          ...
          console.log(
            `  ${pctColor(bar)} ${pctColor(pct + '%')}  ${chalk.white(data.name.padEnd(28))} ${chalk.grey(data.count + ' commits  ' + firstD + ' → ' + lastD)}`,
          );
        }
